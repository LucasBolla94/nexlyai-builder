...
import { startDevServer } from "@/lib/project-scaffolder";
...
export async function POST(
  ...
  try {
    ...
    if (!project.projectPath) {
      return NextResponse.json({ error: "Project not scaffolded yet" }, { status: 400 });
    }

    const success = await startDevServer(id);
    if (!success) {
